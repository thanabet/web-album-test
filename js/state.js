...
export function nextSpread(state) {
  ...
  if (state.spreadIndex < MAX_SPREAD) {
    ...
    // Past the end -> show back cover closed state
    closeToBack(state);
  }
}
...
export function prevSpread(state) {
  if (!state.isOpen) {
    // if currently showing back cover closed, open to last spread
  ...
  if (state.spreadIndex > 0) {
    ...
    // Going back from first spread -> close to front cover
    closeToFront(state);
  }
}
